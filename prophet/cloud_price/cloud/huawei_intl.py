        ...
        for price in on_demand_price_list:
            flavors_dict[price["id"]]["product_rating_result"].update({
                "on_demand": {
                    ...
                    self.huawei_resources["USD_CURRENCY_UNIT"]:
                        price.get("amount"),
                    self.huawei_resources["RMB_CURRENCY_UNIT"]:
                        price.get("amount")
                }
            })
        ...
        for price in on_period_price_list:
            flavors_dict[price["id"]]["product_rating_result"].update({
                "on_period": {
                    ...
                    self.huawei_resources["USD_CURRENCY_UNIT"]: price.get(
                            "official_website_amount"),
                    self.huawei_resources["RMB_CURRENCY_UNIT"]: price.get(
                            "official_website_amount"),
                }
            })
                ...
                if disk_type["name"] == price["id"]:
                    on_period = {
                        "on_period": {
                            ...
                            self.huawei_resources["USD_CURRENCY_UNIT"]:
                                price.get("official_website_amount"),
                            self.huawei_resources["RMB_CURRENCY_UNIT"]:
                                price.get("official_website_amount"),
                        }
                    }
    ...
    def obs_price_processing(self, ak, sk, region, project_id):
        ...
        if price_list:
            obs["storage"]["3az"]["product_rating_result"] = {
                ...
                self.huawei_resources["USD_CURRENCY_UNIT"]: price_list[0].get(
                                "amount"),
                self.huawei_resources["RMB_CURRENCY_UNIT"]: price_list[0].get(
                                "amount"),
            }
